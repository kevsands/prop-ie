...
const AdminFinancialDashboard: React.FC = () => {
      ...
      setTimeout(() => {
        const mockPayments = [
          ...
          },
          {
            ...
            paidDate: new Date(Date.now() - 9 * 24 * 60 * 60 * 1000).toISOString(),
            reference: 'DEP-10003',
            notes: 'Initial booking deposit'
          },
          {
          ...
          },
          {
            ...
            paidDate: new Date(Date.now() - 19 * 24 * 60 * 60 * 1000).toISOString(),
            reference: 'DEP-10004',
            notes: 'Initial booking deposit'
          }
        ];
        ...
        // Calculate summary
        const totalProperties = new Set(filteredPayments.map(p => p.purchaseId)).size;
        ...
        const depositsPaid = filteredPayments.filter(p => p.type === 'deposit' && p.status === 'completed').reduce((sum, p) => sum + p.amount, 0);
        const depositsTotal = filteredPayments.filter(p => p.type === 'deposit').reduce((sum, p) => sum + p.amount, 0);
        
        const installmentsPaid = filteredPayments.filter(p => p.type === 'installment' && p.status === 'completed').reduce((sum, p) => sum + p.amount, 0);
        const installmentsTotal = filteredPayments.filter(p => p.type === 'installment').reduce((sum, p) => sum + p.amount, 0);
        
        const finalPaymentsPaid = filteredPayments.filter(p => p.type === 'final' && p.status === 'completed').reduce((sum, p) => sum + p.amount, 0);
        const finalPaymentsTotal = filteredPayments.filter(p => p.type === 'final').reduce((sum, p) => sum + p.amount, 0);
  ...
  // Mock API function - will be replaced with actual API call
  const processPayment = async (paymentId: string, status: string, notes: string) => {
    // Simulate API call
    return new Promise<any>((resolve) => {
      setTimeout(() => {
        resolve({
  ...
  const handleProcessPayment = async (status: string) => {
    ...
    <div className="bg-white shadow overflow-hidden sm:rounded-lg">
      ...
      <div className="border-t border-gray-200 px-4 py-5 sm:px-6">
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="bg-white p-4 rounded-lg shadow border border-gray-200">
                ...
                <p className="text-2xl font-semibold text-green-600">{formatCurrency(summary.totalPaid)}</p>
              </div>
              <div className="bg-white p-4 rounded-lg shadow border border-gray-200">
                <p className="text-sm text-gray-500">Total Pending</p>
                <p className="text-2xl font-semibold text-yellow-600">{formatCurrency(summary.totalPending)}</p>
              </div>
            </div>
            ...
            <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-white p-4 rounded-lg shadow border border-gray-200">
                ...
                <p className="text-xl font-semibold text-gray-900">
                  {formatCurrency(summary.depositsPaid)} / {formatCurrency(summary.depositsTotal)}
                </p>
                <div className="mt-2 relative pt-1">
                  <div className="overflow-hidden h-2 text-xs flex rounded bg-gray-200">
                    <div
                      style={{ width: `${summary.depositsTotal > 0 ? (summary.depositsPaid / summary.depositsTotal) * 100 : 0}%` }}
              ...
              <div className="bg-white p-4 rounded-lg shadow border border-gray-200">
                ...
                <p className="text-xl font-semibold text-gray-900">
                  {formatCurrency(summary.installmentsPaid)} / {formatCurrency(summary.installmentsTotal)}
                </p>
                <div className="mt-2 relative pt-1">
                  <div className="overflow-hidden h-2 text-xs flex rounded bg-gray-200">
                    <div
                      style={{ width: `${summary.installmentsTotal > 0 ? (summary.installmentsPaid / summary.installmentsTotal) * 100 : 0}%` }}
              ...
              <div className="bg-white p-4 rounded-lg shadow border border-gray-200">
                ...
                <p className="text-xl font-semibold text-gray-900">
                  {formatCurrency(summary.finalPaymentsPaid)} / {formatCurrency(summary.finalPaymentsTotal)}
                </p>
                <div className="mt-2 relative pt-1">
                  <div className="overflow-hidden h-2 text-xs flex rounded bg-gray-200">
                    <div
                      style={{ width: `${summary.finalPaymentsTotal > 0 ? (summary.finalPaymentsPaid / summary.finalPaymentsTotal) * 100 : 0}%` }}
        ...
        <div className="mb-6 flex flex-col md:flex-row md:items-center md:justify-between">
          <h4 className="text-md font-medium text-gray-900 mb-2 md:mb-0">Payment Transactions</h4>
            ...
            <div className="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
              <table className="min-w-full divide-y divide-gray-300">
                <thead className="bg-gray-50">
                  <tr>
                    <th scope="col" className="py-3.5 pl-4 pr-3 text-left text-sm font-semibold text-gray-900 sm:pl-6">Property / Buyer</th>
                ...
                <tbody className="divide-y divide-gray-200 bg-white">
                  {payments.length > 0 ? (
                    payments.map((payment) => (
                      ...
                      >
                        <td className="whitespace-nowrap py-4 pl-4 pr-3 text-sm sm:pl-6">
                          ...
                          <div className="text-gray-500">{payment.buyerName}</div>
                        </td>
                        ...
                        <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getPaymentStatusBadgeClass(payment.status)}`}>
                    ...
                    <tr>
                      <td colSpan={5} className="px-3 py-8 text-sm text-gray-500 text-center">
                ...
                <div className="px-4 py-5 sm:p-6">
                  <dl className="grid grid-cols-1 gap-x-4 gap-y-6">
                    <div>
                      ...
                      <dd className="mt-1 text-sm text-gray-900">{selectedPayment.propertyName}</dd>
                    </div>
                    
                    <div>
                      <dt className="text-sm font-medium text-gray-500">Buyer</dt>
                      <dd className="mt-1 text-sm text-gray-900">{selectedPayment.buyerName}</dd>
                    </div>
                    
                    <div>
                      <dt className="text-sm font-medium text-gray-500">Payment Type</dt>
                      <dd className="mt-1 text-sm text-gray-900">{getPaymentTypeText(selectedPayment.type)}</dd>
                    </div>
                    ...
                    <div>
                      ...
                      <dd className="mt-1 text-sm text-gray-900">{formatDate(selectedPayment.dueDate)}</dd>
                    </div>
                    
                    {selectedPayment.paidDate && (
                      <div>
                        <dt className="text-sm font-medium text-gray-500">Paid Date</dt>
                        <dd className="mt-1 text-sm text-gray-900">{formatDate(selectedPayment.paidDate)}</dd>
                      </div>
                    )}
                    ...
                    <div>
                      ...
                      <dd className="mt-1">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getPaymentStatusBadgeClass(selectedPayment.status)}`}>
                    ...
                    {selectedPayment.reference && (
                      <div>
                        ...
                        <dd className="mt-1 text-sm text-gray-900">{selectedPayment.reference}</dd>
                      </div>
                    )}
