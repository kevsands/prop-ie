...
const AdminDocumentReview: React.FC = () => {
        ...
        resolve({
          ...
          data: [
            ...
            },
            {
              ...
              status: 'rejected',
              notes: 'Document is incomplete. Please provide full solicitor details.',
  ...
  // Mock API function - will be replaced with actual API call
  const reviewDocument = async (documentId: string, status: string, notes: string) => {
    // Simulate API call
    return new Promise<any>((resolve) => {
      setTimeout(() => {
        resolve({
  ...
  const handleReviewStatusChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
  ...
  return (
    <div className="bg-white shadow overflow-hidden sm:rounded-lg">
      <div className="px-4 py-5 sm:px-6">
        <h3 className="text-lg leading-6 font-medium text-gray-900">Document Review</h3>
          ...
          <div className="lg:col-span-1 overflow-hidden">
            ...
            {documents.length > 0 ? (
                      ...
                      >
                        <div className="flex items-center">
                          ...
                          <div className="ml-3">
                            ...
                            <p className="text-xs text-gray-500">{getDocumentTypeText(document.type)}</p>
              ...
              <div className="text-center py-6 bg-white rounded-lg border border-gray-200">
                ...
                <p className="mt-1 text-sm text-gray-500">No documents match the selected filter.</p>
              </div>
            )}
          </div>
          ...
          <div className="lg:col-span-2">
            {selectedDocument ? (
              <div>
                ...
                {reviewSuccess && (
                  <div className="bg-green-50 border-l-4 border-green-400 p-4 mb-6">
                    <div className="flex">
                      <div className="ml-3">
                        <p className="text-sm text-green-700">Document review submitted successfully!</p>
                      </div>
                    </div>
                  </div>
                )}
                ...
                <div className="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6 rounded-t-lg">
                  ...
                  <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{selectedDocument.filename}</dd>
                </div>
                
                <div className="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                  <dt className="text-sm font-medium text-gray-500">Document Type</dt>
                  <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{getDocumentTypeText(selectedDocument.type)}</dd>
                </div>
                
                <div className="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                  <dt className="text-sm font-medium text-gray-500">Property</dt>
                  <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{selectedDocument.propertyName}</dd>
                </div>
                
                <div className="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                  <dt className="text-sm font-medium text-gray-500">Buyer</dt>
                  <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{selectedDocument.buyerName}</dd>
                </div>
                
                <div className="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                  <dt className="text-sm font-medium text-gray-500">Upload Date</dt>
                  <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{new Date(selectedDocument.uploadDate).toLocaleString()}</dd>
                </div>
                
                <div className="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                  <dt className="text-sm font-medium text-gray-500">Current Status</dt>
                  <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getDocumentStatusBadgeClass(selectedDocument.status)}`}>
                ...
                <div className="bg-gray-50 px-4 py-5 sm:px-6 rounded-b-lg">
                  <h5 className="text-sm font-medium text-gray-900 mb-3">Document Preview</h5>
                ...
                <form onSubmit={handleReviewSubmit} className="mt-6">
                  <h4 className="text-md font-medium text-gray-900 mb-4">Review Document</h4>
                  
                  <div className="mb-4">
                    <label htmlFor="reviewStatus" className="block text-sm font-medium text-gray-700">
                      Review Status
                    </label>
                    <select
                      id="reviewStatus"
                  ...
                  <div className="mb-4">
                    ...
                    <p className="mt-2 text-xs text-gray-500">
                      {reviewStatus === 'rejected' ? 'Please provide a reason for rejection.' : 'Optional notes for this review.'}
                    </p>
                  </div>
              ...
              <div className="text-center py-12 bg-gray-50 rounded-lg border border-gray-200">
                ...
                <p className="mt-1 text-sm text-gray-500">Select a document from the list to review it.</p>
              </div>
            )}
          </div>
        </div>
