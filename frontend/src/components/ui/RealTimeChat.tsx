...
const RealTimeChat: React.FC<RealTimeChatProps> = ({ purchaseId, propertyName }) => {
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (user) {
      ...
      socketInstance.emit('join_chat', { purchaseId }, (response: { messages: ChatMessage[] }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-white shadow-lg rounded-lg">
      <div className="px-4 py-3 border-b border-gray-200">
        <h3 className="text-lg font-medium text-gray-900">Chat - {propertyName}</h3>
