...
const PurchaseList: React.FC = () => {
  ...
  if (purchases.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <p className="mt-1 text-sm text-gray-500">You haven't made any property purchases yet.</p>
        <div className="mt-6">
          <Link href="/property">
            <button
              type="button"
  ...
  return (
    <div>
      <h2 className="text-lg font-medium text-gray-900 mb-4">Your Property Purchases</h2>
                  ...
                  <div className="flex items-center px-4 py-4 sm:px-6">
                    <div className="min-w-0 flex-1 flex items-center">
                      ...
                      <div className="min-w-0 flex-1 px-4">
                        <div>
                          <p className="text-sm font-medium text-blue-600 truncate">{purchase.property.name}</p>
                          <p className="mt-1 flex items-center text-sm text-gray-500">
                            <span className="truncate">€{purchase.property.price.toLocaleString()}</span>
                          </p>
                          <p className="mt-1 flex items-center text-sm text-gray-500">
                            <span className="truncate">Booking Date: {purchase.bookingDate}</span>
                          </p>
                        </div>
                      </div>
                    </div>
                    <div className="ml-5 flex-shrink-0 flex flex-col items-end">
                      ...
                      <div className="mt-2 flex items-center text-sm text-gray-500">
                        {purchase.bookingDepositPaid ? (
                          <span className="text-green-600 text-xs">Deposit Paid</span>
                        ) : (
                          <span className="text-yellow-600 text-xs">Deposit Pending</span>
                        )}
                      </div>
                    </div>
                  </div>
