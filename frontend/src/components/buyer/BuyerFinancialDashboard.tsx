  ...
  if (paymentSchedules.length === 0) {
    return (
      <div className="text-center py-12 bg-white rounded-lg shadow">
        ...
        <h3 className="mt-2 text-sm font-medium text-gray-900">No payment schedules</h3>
        <p className="mt-1 text-sm text-gray-500">You don't have any active payment schedules.</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white shadow overflow-hidden sm:rounded-lg">
      <div className="px-4 py-5 sm:px-6">
        <h3 className="text-lg leading-6 font-medium text-gray-900">Financial Dashboard</h3>
      ...
      <div className="border-t border-gray-200 px-4 py-5 sm:px-6">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-1">
            <h4 className="text-md font-medium text-gray-900 mb-4">Your Properties</h4>
                    ...
                    >
                      <div className="flex flex-col">
                        <div className="flex justify-between">
                          <p className="text-sm font-medium text-blue-600">{schedule.propertyName}</p>
                          <p className="text-sm font-medium text-gray-900">{formatCurrency(schedule.totalAmount)}</p>
                        </div>
                        <div className="mt-2">
                          <div className="relative pt-1">
                            <div className="overflow-hidden h-2 text-xs flex rounded bg-gray-200">
                              <div
                                style={{ width: `${calculateProgress(schedule.paidAmount, schedule.totalAmount)}%` }}
                                className="shadow-none flex flex-col text-center whitespace-nowrap text-white justify-center bg-blue-500"
                              ></div>
                            </div>
                          </div>
                          <div className="flex justify-between text-xs text-gray-500 mt-1">
                            <span>Paid: {formatCurrency(schedule.paidAmount)}</span>
                            <span>{calculateProgress(schedule.paidAmount, schedule.totalAmount)}%</span>
                          </div>
                        </div>
                        {schedule.nextPaymentDate && (
                          <p className="mt-2 text-xs text-gray-500">
                            Next payment: {formatCurrency(schedule.nextPaymentAmount || 0)} due on {formatDate(schedule.nextPaymentDate)}
                          </p>
                        )}
                      </div>
                    </button>
            ...
            {selectedSchedule && (
              <div>
                <div className="flex justify-between items-center mb-4">
                  <h4 className="text-md font-medium text-gray-900">{selectedSchedule.propertyName} - Payment Schedule</h4>
                  <button
                    type="button"
                ...
                <div className="bg-gray-50 p-4 rounded-lg mb-6">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="bg-white p-4 rounded shadow">
                      ...
                      <p className="text-xl font-semibold text-gray-900">{formatCurrency(selectedSchedule.totalAmount)}</p>
                    </div>
                    <div className="bg-white p-4 rounded shadow">
                      <p className="text-sm text-gray-500">Paid Amount</p>
                      <p className="text-xl font-semibold text-green-600">{formatCurrency(selectedSchedule.paidAmount)}</p>
                    </div>
                    <div className="bg-white p-4 rounded shadow">
                      <p className="text-sm text-gray-500">Remaining Amount</p>
                      <p className="text-xl font-semibold text-blue-600">{formatCurrency(selectedSchedule.remainingAmount)}</p>
                    </div>
                  </div>
                      ...
                      <div className="flex mb-2 items-center justify-between">
                        ...
                        <div className="text-right">
                          <span className="text-xs font-semibold inline-block text-blue-600">
                            {calculateProgress(selectedSchedule.paidAmount, selectedSchedule.totalAmount)}%
                          </span>
                        </div>
                      </div>
                      <div className="overflow-hidden h-2 mb-4 text-xs flex rounded bg-blue-200">
                        <div
                          style={{ width: `${calculateProgress(selectedSchedule.paidAmount, selectedSchedule.totalAmount)}%` }}
                          className="shadow-none flex flex-col text-center whitespace-nowrap text-white justify-center bg-blue-500"
                        ></div>
                      </div>
                    </div>
                    ...
                    <div className="flex">
                      ...
                      <div className="ml-3 flex-1 md:flex md:justify-between">
                        <p className="text-sm text-blue-700">
                          Your next payment of {formatCurrency(selectedSchedule.nextPaymentAmount || 0)} is due on {formatDate(selectedSchedule.nextPaymentDate)}.
                        </p>
